...
export function Settings() {
  ...
  const exportLedger = () => {
    ...
    Object.assign(document.createElement('a'), { href: url, download: `zkrail-ledger-${shortAddr(wallet, 6, 4)}.json` }).click();
  ...
  const importLedger = async (file: File) => {
    try {
      ...
      setNotice(ok ? 'Ledger restored from backup.' : 'That file is not a ZKRail ledger backup.');
  ...
  return (
    <>
      <PageHead index="08" title="Settings" lede="Your store, where payouts land, and how ZKRail talks to your backend." />
      ...
      <form className="dash-panel dash-form" onSubmit={save}>
        ...
        <div className="dash-grid-2 dash-grid-2--tight">
          <label className="dash-field">
            ...
            <input className="dash-input" value={name} onChange={(e) => setName(e.target.value)} maxLength={60} placeholder="ZKRail Demo Store" />
          </label>
          <label className="dash-field">
        ...
        <label className="dash-field">
          ...
          <input className="dash-input num" value={hook} onChange={(e) => setHook(e.target.value.trim())} placeholder="https://your-store.com/hooks/zkrail" spellCheck={false} aria-invalid={!hookValid} />
