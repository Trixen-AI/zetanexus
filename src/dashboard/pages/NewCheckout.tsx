    ...
    try {
      ...
      if (apiConfigured()) {
        const remote = await createRemoteCheckout({
          merchant_name: ledger.profile.merchantName || 'ZetaNexus merchant',
      ...
      <div className="dash-grid-form">
        ...
        <aside className="dash-panel dash-quote">
          ...
          <div className={`dash-api-note${apiConfigured() ? ' is-on' : ''}`}>
            <IconLock size={13} />
            {apiConfigured()
              ? 'The ZetaNexus API will issue a single-use shielded address for this checkout.'
              : 'No checkout API configured: this checkout gets a quote and on-chain settlement tracking, but no shielded ZEC address until VITE_ZETANEXUS_API_URL is set.'}
          </div>
